...
import { getServerStatus } from './apis';
...
ipcMain.on('getConfiguration', async (event, arg) => {
  //read file config.json
  const configFile = getConfigFile();
  console.log('configFile:', configFile);
  event.reply('getConfiguration', { data: configFile });
});

ipcMain.on('getTranslatedData', async (event, arg) => {
  // read translation file
  console.log('getTranslatedData: arg:', arg[0]);
  const translation = await getTranslatedData(arg[0]['arg']);
  console.log('translation:', translation);
  event.reply('getTranslatedData', { data: translation });
});
ipcMain.on('getReferenceData', onRequestReferenceData);
